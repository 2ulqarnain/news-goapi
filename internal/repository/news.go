...
func (r *NewsRepository) GetAllNews(limit int) ([]model.News, error) {
	var (
		dbQuery string
		rows    *sql.Rows
		err     error
	...
	if limit > 0 {
		...
		rows, err = r.db.Query(dbQuery, limit)
	} else {
		dbQuery = "SELECT * FROM news"
		rows, err = r.db.Query(dbQuery)
	}
	if err != nil {
		return nil, err
	}
...
func (r *NewsRepository) GetNewsBySlug(slug string) (*model.News, error) {
	query := "SELECT slug, title, published_on, news_url, image_url FROM news WHERE slug=?"
	...
	var news model.News
	for rows.Next() {
		err := rows.Scan(&news.Slug, &news.Title, &news.PublishedOn, &news.NewsUrl, &news.ImageUrl)
		if err != nil {
			log.Printf("Couldn't load data from database!, Error: %v", err)
			return nil, err
		}
...
func (r *NewsRepository) AddSingleNews(news model.News) {
	tx, err := r.db.Begin()
	if err != nil {
		fmt.Println("Failed to begin transaction:", err)
		return
